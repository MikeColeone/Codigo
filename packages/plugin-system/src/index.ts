...
class ComponentRegistry {
  ...
  public register<T extends string, P extends Record<string, any>, R>(
    ...
    if (this.plugins.has(plugin.type)) {
      console.warn(
        `[plugin-system] Plugin with type "${plugin.type}" is already registered. It will be overwritten.`,
      );
    }
  ...
  public getDefaultConfigs(): Record<string, TBasicComponentConfig> {
    ...
    this.plugins.forEach((plugin, key) => {
      configs[key] = plugin.defaultConfig as TBasicComponentConfig;
    });
    return configs;
  }
}
...
export const getComponentDefaultConfigs =
  registry.getDefaultConfigs.bind(registry);
